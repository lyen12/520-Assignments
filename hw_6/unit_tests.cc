...
//     std::cout << "Total time is " << w.get_minutes() << " minutes\n";
//     SLEEP;
//     std::cout << "Total time is " << w.get_seconds() << "\n"; // still about 1.5
//     w.start(); 
//     SLEEP;
...
TEST_P(StopwatchTests, IntermediateTest) {
    ...
    SLEEP_THREAD(x);
    SLEEP_THREAD(x);
    cout << "Minutes is " << w.get_minutes() << " Expected is " <<  2*x/60000.0 << "\n";
...
TEST_P(StopwatchTests, AdvancedTest) {
    ...
    w.start();
    SLEEP_THREAD(x);
        cout << "ADV seconds is " << w.get_minutes() << " Expected is " <<  x/60000.0 << "\n";
...
TEST_P(RunningAverageTests, RunningAverage) {
    ...
    m.schedule(r, 1_ms)
     ...
     .init()
     .run(dur);
     cout << "My result is " << f.value() << " Expected results is " << result << "\n";
