...
static int current_allocated_stack_size;
static double * stack;
...
void rpn_init() {
  if ( ! initialized ) {
      ...
      top = 0;
      error = OK;
      current_allocated_stack_size = INITIAL_STACK_SIZE;
  }
}
...
void rpn_push(double x) {
    ...
    } else if ( top == current_allocated_stack_size ) {
        current_allocated_stack_size *= STACK_GROW_FACTOR;
        stack = (double *)realloc(stack,current_allocated_stack_size*sizeof(double));
        stack[top] = x;
        top++;
    } else {
        stack[top] = x;
