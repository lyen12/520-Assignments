...
/*returns all the prime numbers less than or equal to n using 
...
vector<int> primes(int n) {
	...
	auto numbers = create_vector_to_n(n);		//initiate list
	while( another_prime == true ) {
			...
			another_prime = false;		//forces the next condition to be true to continue loop
...
vector<tuple<int,int>> twins(vector<int> nums) {
	...
	/*Then for each element, we will check if any elements is 2+ than that 
	element*/
	/*This for loop will check the current number and next number*/
	...
	/*This for loop will check the current number and number two next over.
	This separate loop was made to avoid checking past the vector, hence new
	stopping point*/ 
